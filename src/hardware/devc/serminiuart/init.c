...
void
create_device(TTYINIT * const dip)
{
    DEV_MINIUART	*dev;
    ...
    dev->tty.ibuf.buff = malloc(dev->tty.ibuf.size);
    ...
    dev->tty.obuf.buff = malloc(dev->tty.obuf.size);
    ...
    dev->tty.cbuf.buff = malloc(dev->tty.cbuf.size);
