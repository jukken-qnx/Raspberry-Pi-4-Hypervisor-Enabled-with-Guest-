...
typedef struct _rcar_sdmmc_t {
    ...
    uint32_t        pclk;
    int             blksz;
    uint32_t        busclk;       // MMC bus clock
    uint32_t        scc_min_clk;  // SCC min clock
    uintptr_t       vbase;
    uint32_t        flags;
...
static inline uint32_t sdmmc_read(uintptr_t const base, const uint32_t reg)
{
    return in32(base + reg);
}

static inline int rcar_sdmmc_check_sclkdiven(const uintptr_t base)
{
    int i;
...
static inline void sdmmc_write(const uintptr_t base, const uint32_t reg, uint32_t val)
{
...
static inline void sdmmc_bitset(const uintptr_t base, const uint32_t reg, const uint32_t val)
{
    out32(base + reg, val | in32(base + reg));
}

static inline void sdmmc_bitclr(const uintptr_t base, const uint32_t reg, const uint32_t val)
{
    out32(base + reg, ~val & in32(base + reg));
}
