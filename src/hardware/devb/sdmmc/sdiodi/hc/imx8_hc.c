 ...
 * @return EOK always.
 */
static int imx_sdhcx_pwr(sdio_hc_t *const hc, const uint32_t vdd)
{
    const imx_sdhcx_hc_t    *sdhc;
    uintptr_t               base;
    uint32_t                reg;
 ...
 * @return EOK always.
 */
static int imx_sdhcx_bus_mode(sdio_hc_t *const hc, const uint32_t bus_mode)
{
 ...
 * @return EOK always.
 */
static int imx_sdhcx_bus_width(sdio_hc_t *const hc, const uint32_t width)
{
    const imx_sdhcx_hc_t     *sdhc;
    uintptr_t                base;
    uint32_t                 hctl;
 ...
 * @return EOK always.
 */
static int imx_sdhcx_clk(sdio_hc_t *hc, uint32_t clk)
{
    imx_sdhcx_hc_t         *sdhc;
    uintptr_t               base;
    uint32_t                sctl;
    ...
    while ((hc->clk_max / (pre_div * divd) > clk) && (divd < 16)) {
        divd++;
    }
 ...
 * @return EOK always.
 */
static int imx_sdhcx_timing(sdio_hc_t *hc, const uint32_t timing)
{
    const imx_sdhcx_hc_t     *sdhc;
    uintptr_t                base;
    uint32_t                 mix_ctl;
 ...
 * @return Execution status.
 */
static int imx_sdhcx_signal_voltage(sdio_hc_t *hc, const uint32_t signal_voltage)
{
    const imx_sdhcx_hc_t      *sdhc;
    uintptr_t                 base;
    uint32_t                  reg, dlsl;
    ...
    if ((hc->version < IMX_USDHC_HOST_CTRL_VER_SVN_BV_VER_3) || (hc->signal_voltage == signal_voltage)) {
        return (EOK);
    }
 ...
 * @return Execution status.
 */
static int imx_sdhcx_send_tune_cmd(sdio_hc_t *hc, const uint32_t op)
{
    struct sdio_cmd     cmd;
    uint32_t            tlen;
    int                 status;
    ...
    tlen = (hc->bus_width == BUS_WIDTH_8) ? 128U : 64U;
 ...
 * @return Execution status.
 */
static int imx_sdhcx_std_tune(sdio_hc_t *hc, const uint32_t op)
{
    const imx_sdhcx_hc_t     *sdhc;
    uint32_t                 *buf;
    struct sdio_cmd          *cmd;
    ...
    for (i = 0; i < IMX_USDHC_TUNING_RETRIES; i++) {
        ...
        /* Empty buffer */
        in32s(buf, (tlen >> 2), base + IMX_USDHC_DATA_BUFF_ACC_PORT);
 ...
 * @return Execution status.
 */
static int imx_sdhcx_man_tune(sdio_hc_t *const hc, const uint32_t op)
{
    const imx_sdhcx_hc_t    *sdhc;
    uintptr_t               base;
    uint32_t                mix_ctl;
 ...
 * @return Execution status.
 */
static int imx_sdhcx_tune(sdio_hc_t *const hc, const uint32_t op)
{
    int status = EOK;
    const imx_sdhcx_hc_t *const sdhc = hc->cs_hdl;
