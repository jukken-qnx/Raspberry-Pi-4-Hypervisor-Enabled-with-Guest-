...
static int dw_set_ldo( sdio_hc_t *hc, int ldo, uint32_t voltage )
{
	...
	status = bs_set_ldo( hc, ldo, voltage );
	if( status != EOK ) {
...
static int dw_pm( sdio_hc_t *hc, uint32_t op )
{
#ifdef DW_DEBUG
...
static int dw_pwr( sdio_hc_t *hc, uint32_t vdd )
{
	dw_hc_msh_t		*msh;
	uintptr_t		base;
...
static int dw_bus_mode( sdio_hc_t *hc, uint32_t bus_mode )
{
#ifdef DW_DEBUG
...
static int dw_bus_width( sdio_hc_t *hc, uint32_t width )
{
	dw_hc_msh_t		*msh;
	uintptr_t		base;
	uint32_t		ctype;
...
static int dw_clk( sdio_hc_t *hc, uint32_t clk )
{
	dw_hc_msh_t		*msh;
	uintptr_t		base;
	uint32_t		divisor;
...
static int dw_timing( sdio_hc_t *hc, uint32_t timing )
{
	dw_hc_msh_t		*msh;
	uintptr_t		base;
	uint32_t		uhs;
...
static int dw_signal_voltage( sdio_hc_t *hc, uint32_t signal_voltage )
{
	dw_hc_msh_t			*msh;
	uintptr_t			base;
	uint32_t			uhs;
...
static int dw_drv_type( sdio_hc_t *hc, uint32_t type )
{
#ifdef DW_DEBUG
...
static int dw_tune( sdio_hc_t *hc, uint32_t op )
{
	dw_hc_msh_t		*msh;
	sdio_cmd_t		*cmd;
	sdio_sge_t		sge;
...
static int dw_preset( sdio_hc_t *hc, uint32_t enable )
{
	(void) hc;
	(void) enable;
#ifdef DW_DEBUG
...
int dw_init( sdio_hc_t *hc )
{
	...
	hc->entry			= (sdio_hc_entry_t) {	.nentries			= SDIO_HC_ENTRY_NFUNCS,
												.dinit				= dw_dinit,
												.pm					= NULL,
												.cmd				= dw_cmd,
												.abort				= dw_abort,
