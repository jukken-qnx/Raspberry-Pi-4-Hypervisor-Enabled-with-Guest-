...
static int ls10xx_sdhcx_pwr(sdio_hc_t *hc, uint32_t vdd)
{
...
static int ls10xx_sdhcx_bus_mode(sdio_hc_t *hc, uint32_t bus_mode)
{
    hc->bus_mode = bus_mode;
    return (EOK);
}

static int ls10xx_sdhcx_bus_width(sdio_hc_t *hc, uint32_t width)
{
...
static int ls10xx_sdhcx_clk(sdio_hc_t *hc, uint32_t clk)
{
    ls10xx_hc_t *sdhc  = (ls10xx_hc_t *) hc->cs_hdl;
...
static int ls10xx_sdhcx_timing(sdio_hc_t *hc, uint32_t timing)
{
...
static int ls10xx_sdhcx_signal_voltage(sdio_hc_t *hc, uint32_t signal_voltage)
{
...
static int ls10xx_send_tune_cmd(sdio_hc_t *hc, uint op)
{
    struct sdio_cmd  cmd;
...
static int ls10xx_sdhcx_tune_errata_a008171_wa(sdio_hc_t *hc, int op, int hw_tuning_result)
{
    ...
    LS10XX_LOG(hc, _SLOG_INFO, "Tuning for errata A-008171");
...
static int ls10xx_sdhcx_tune(sdio_hc_t *hc, uint32_t op)
{
    int status = EOK;
    int  retries = 0;
    uint32_t data;
